...
public interface EstimationRepository extends CrudRepository<Estimation, Integer> {
    ...
    @Query(value = "SELECT id_estimation AS idEstimation, estimation AS estimation, id_requirement AS idRequirement, id_user AS idUser FROM estimation WHERE estimation.id_user = 4", nativeQuery = true)
    List<EstimationDto2> checkIfEstimationAlreadyExist();
